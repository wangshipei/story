...
微信在跑、只是主窗口被关掉时（macOS 上关窗口不退程序），发送前会用 `open -b com.tencent.xinWeChat` 把主窗口唤回来，每次尝试只唤一次，唤不回才按未就绪退出等下一轮。别改回 AppleScript（`tell application "WeChat" to reopen/activate` 在这台 Mac 上对微信无效，返回 0 但窗口不回来）；微信没在跑、缺辅助功能或只缺屏幕录制时都不会去唤。
